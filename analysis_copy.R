...
build_arimax_specs <- function(train_df, test_df, predictor_pool, max_k = 3, include_full = TRUE) {
  ...
  lapply(combos, function(vars) {
    list(
      train = as.matrix(train_df[, vars, drop = FALSE]),
      test = as.matrix(test_df[, vars, drop = FALSE]),
...
best_can_arimax_train <- can_arimax_specs[[best_can_arimax_idx]]$train
best_can_arimax_test  <- can_arimax_specs[[best_can_arimax_idx]]$test
best_can_arimax_name  <- can_arimax_specs[[best_can_arimax_idx]]$description
...
best_us_arimax_train <- us_arimax_specs[[best_us_arimax_idx]]$train
best_us_arimax_test  <- us_arimax_specs[[best_us_arimax_idx]]$test
best_us_arimax_name  <- us_arimax_specs[[best_us_arimax_idx]]$description
